...
public class InMemoryAppBusinessSettingsRepository implements AppBusinessSettingsRepository {
    ...
    @Override
    public Mono<AppBusinessSettings> findByOrganizationId(UUID tenantId, UUID organizationId) {
        return Mono.justOrEmpty(settings.get(key(tenantId, organizationId)));
    }

    @Override
    public Mono<AppBusinessSettings> save(AppBusinessSettings appBusinessSettings) {
        return Mono.fromSupplier(() -> {
            settings.put(key(appBusinessSettings.tenantId(), appBusinessSettings.organizationId()), appBusinessSettings);
            return appBusinessSettings;
        });
    }

    private String key(UUID tenantId, UUID organizationId) {
