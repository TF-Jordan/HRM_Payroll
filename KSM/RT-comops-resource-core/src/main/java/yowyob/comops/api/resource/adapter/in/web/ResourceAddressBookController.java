...
public class ResourceAddressBookController {
    ...
    public ResourceAddressBookController(GetMaterialResourceUseCase getMaterialResourceUseCase,
    ...
    public Mono<ResponseEntity<ApiResponse<AddressResponse>>> createAddress(@PathVariable UUID resourceId,
        ...
        return requestMono.zipWith(ReactiveRequestContextHolder.getRequiredContext())
                .flatMap(tuple -> ensureResourceExists(resourceId)
                        .then(createAddressUseCase.createAddress(new CreateAddressCommand(
                                tuple.getT2().tenantId(), AddressableType.RESOURCE, resourceId,
                ...
                .map(response -> ResponseEntity.status(HttpStatus.CREATED)
                        .body(ApiResponse.success(response, "Resource address created.")));
    }

    @GetMapping("/addresses")
    public Mono<ResponseEntity<ApiResponse<List<AddressResponse>>>> listAddresses(@PathVariable UUID resourceId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context -> ensureResourceExists(resourceId)
                        .then(listAddressesUseCase.listAddresses(context.tenantId(), AddressableType.RESOURCE, resourceId)
                                .map(AddressResponse::from)
                                .collectList()))
                .map(response -> ResponseEntity.ok(ApiResponse.success(response, "Resource addresses retrieved.")));
    }

    @DeleteMapping("/addresses/{addressId}")
    public Mono<ResponseEntity<ApiResponse<Void>>> deleteAddress(@PathVariable UUID resourceId,
            @PathVariable UUID addressId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context -> ensureResourceExists(resourceId)
                        .then(deleteAddressUseCase.deleteAddress(context.tenantId(), addressId)))
                .thenReturn(ResponseEntity.ok(ApiResponse.success(null, "Resource address deleted.")));
    }

    @PostMapping("/contacts")
    public Mono<ResponseEntity<ApiResponse<ContactResponse>>> createContact(@PathVariable UUID resourceId,
            @Valid @RequestBody Mono<NestedContactRequest> requestMono) {
        return requestMono.zipWith(ReactiveRequestContextHolder.getRequiredContext())
                .flatMap(tuple -> ensureResourceExists(resourceId)
                        .then(createContactUseCase.createContact(new CreateContactCommand(
                                tuple.getT2().tenantId(), ContactableType.RESOURCE, resourceId,
                ...
                .map(response -> ResponseEntity.status(HttpStatus.CREATED)
                        .body(ApiResponse.success(response, "Resource contact created.")));
    }

    @GetMapping("/contacts")
    public Mono<ResponseEntity<ApiResponse<List<ContactResponse>>>> listContacts(@PathVariable UUID resourceId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context -> ensureResourceExists(resourceId)
                        .then(listContactsUseCase.listContacts(context.tenantId(), ContactableType.RESOURCE, resourceId)
                                .map(ContactResponse::from)
                                .collectList()))
                .map(response -> ResponseEntity.ok(ApiResponse.success(response, "Resource contacts retrieved.")));
    }

    @DeleteMapping("/contacts/{contactId}")
    public Mono<ResponseEntity<ApiResponse<Void>>> deleteContact(@PathVariable UUID resourceId,
            @PathVariable UUID contactId) {
        return ReactiveRequestContextHolder.getRequiredContext()
                .flatMap(context -> ensureResourceExists(resourceId)
                        .then(deleteContactUseCase.deleteContact(context.tenantId(), contactId)))
                .thenReturn(ResponseEntity.ok(ApiResponse.success(null, "Resource contact deleted.")));
    }

    private Mono<Void> ensureResourceExists(UUID resourceId) {
        return getMaterialResourceUseCase.getResource(resourceId).then();
    }
